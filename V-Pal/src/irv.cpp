...
class IRV:public Election {
	...
	public:
    	...
    	int FindWinner (std::vector<string> & files){
			int numBallots = 0;
			int ballotno = 0;
			// We need to loop through all the file
			...
			for(int idx = 0; idx < files.size(); idx++){
				...
				if (file.is_open ()){
					...
					audit.auditPrint ("Number of ballots after this file is: " + std::to_string (numBallots));
					...
					while (std::getline (file, data)){
						...
						for (std::string::size_type p0 = 0, p1 = input.find (','); p1 != std::string::npos || p0 != std::string::npos; (p0 = (p1 == std::string::npos) ? p1 : ++p1), p1 = input.find (',', p0)){
							output.push_back (strtol (input.c_str () + p0, NULL, 0));
						}
						string ballot_data_str = "";
						int e = 0;
						while (e < numCandidates){
							ballot_data_str = ballot_data_str + std::to_string (output[e]);
							e = e + 1;
						}
					...
					audit.auditPrint ("Generated a total of " + std::to_string (numBallots) + " ballots");
					// Close csv
					file.close ();
				} else {
			...
			while (winnerNotFound){
				...
				while (pw < candidates.size ()){
					...
					if (candidates[pw]->CheckInRace ()){
						...
						audit.auditPrint (print_cand_name + " has " + print_cand_votes + " ballots");
					}	
					pw = pw + 1;
				}
				...
				bool majority = (votes[V[numCandidates - 1]] > (GetTotalBallots () / 2));
				// if majority have found the winner. Just return the name of that candidate
				if (majority){
					winnerNotFound = false;	
					int winnerIndex = V[numCandidates - 1];	
					winner = (*candidates[winnerIndex]);	
					audit.auditPrint ("We found a majority, Candidate " + winner.GetName () + " has " + std::to_string (votes[V[numCandidates - 1]]) + " ballots");	
					return 0;
				}
					...
					while (j < numCandidates){
						...
						if (candidates[V[j]]->CheckInRace ()){
							if (votes[V[j]] < votes[V[numCandidates - 1]]){
								...
								while (pe < (candidates[V[j]]->GetMyBallots ().size ())){
									if (candidates[V[j]]->GetMyBallots ()[pe] == 1){
										transferredBallots = transferredBallots + std::to_string (pe) + ",";			
									}			
									pe = pe + 1;	
								}
											
								audit.auditPrint ("We removed Candidate " + candidates[V[j]]->GetName () + ". Their votes are being redistributed");
								audit.auditPrint("Following ballots should be redistributed");		
								...
								while (pq < cand_total){
									if (candidates[pq]->CheckInRace ()){		
										allcandstie = allcandstie + candidates[pq]->GetName () + " , ";
									}			
									pq = pq + 1;	
								}
								...
								audit.auditPrint ("We randomly chose " + winner.GetName () + " as the winner.");
								return 0;		
							}	
						}	
						j = j + 1;
         ...
         *
         */ 
    	void RedistributeVotes (int candidate_id_remove){
			std::vector < int > candidate_removed_ballots;
			//access the ballots of the removed candidate
			candidate_removed_ballots = candidates[candidate_id_remove]->GetMyBallots ();
			int i = 0;
			while (i < candidate_removed_ballots.size ()){
				...
				if (candidate_removed_ballots[i] == 1) {
					...
					std::vector < int > updated_myBallot = candidates[candidate_id_remove]->GetMyBallots ();	
					updated_myBallot[i] = 0;
					//make myBallot[i]=0 for the removed candidate
					candidates[candidate_id_remove]-> SetMyBallots (updated_myBallot);
					...
					if (!ballots[i]->Valid ()){
						audit.auditPrint ("Ballot" + std::to_string (i) + " is now exhausted");	
					}
				
					while (ballots[i]->Valid () && !new_cand_assigned){
							
						int newPref = ballots[i]->GetCurrentPref ();	
						if (candidates[newPref]->CheckInRace ()){
							audit.auditPrint ("Ballot " + std::to_string (i) + " is being given to " + candidates[newPref]->GetName ());
									
							std::vector < int > new_pref_myBallots = candidates[newPref]->GetMyBallots ();			
         ...
         *
         */ 
    	void AddBallot (IRVBallot * newBallot) {
         ...
         *
         */ 
  		std::vector < int > TransformVote (std::vector < int >vote){
    
			std::vector < int > transformedVoteold;
         ...
         *
         */ 
		void GenerateBallots (std::vector < int >vote, int id){
			//skip to the line where the ballot data is stored
			std::vector < int > transformedVote;
			transformedVote = TransformVote (vote);
			IRVBallot * newBallot_ = new IRVBallot (id, transformedVote, numCandidates);
         ...
         *
         */ 
    	void GenerateCandidates (string cand, char p, int id){
		...
		void UpdateCandidates(){
			...
			while (j < candidates.size ()){
				std::vector < int > new_candidate_myBallots;
				int i = 0;
