...
string FindType (string filename){
    ...
    if (infile){ // if exists then return that ballot was found
        cout << "\n" << endl;
        cout << "Ballot file found..." << endl;
    }
    else{ // if not then return that ballot not found and prompt for file check. exit on failure
        cout << "\n" << endl;
        cout << "ERROR: Ballot file not found..." << endl;
        cout << "Please make sure that the csv ballot file is in the current directory " << endl;
        exit (EXIT_FAILURE);
    }
 ...
 * 
 */ 
void IRVDisplay (string person, char party, int votes, int ballots){
    ...
    //print out resuls to the screen
    cout << "RESULTS: \n" << endl;
 ...
 */ 
void Run (string type, std::vector<string> & files){
    ...
    if (type.compare (irv_type) == 0){
        ...
        audit.auditPrint ("Number of votes they received: " + std::to_string (votes));
        ...
        cout << "Incorrect election type compatibility with V-Pal's system" << endl;
        exit (EXIT_FAILURE);
    }
}
